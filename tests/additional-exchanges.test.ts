...
import { hyperliquidControlMatchesTimeframe, rectIntersectsViewport } from '../src/sites/set-timeframe';
import { supportsMultiTimeframeAnalysis } from '../src/sites/capabilities';

describe('additional exchange URL support', () => {
  it('disables multi-timeframe capture only for 10jqka', () => {
    expect(supportsMultiTimeframeAnalysis('10jqka')).toBe(false);
    expect(supportsMultiTimeframeAnalysis('bitget')).toBe(true);
    expect(supportsMultiTimeframeAnalysis('mexc')).toBe(true);
    expect(supportsMultiTimeframeAnalysis('vergex')).toBe(true);
  });
  it('rejects Coinbase range controls outside the Financial Chart viewport', () => {
    expect(rectIntersectsViewport({ top: 1094, right: 120, bottom: 1132, left: 80 }, 1200, 704)).toBe(false);
    expect(rectIntersectsViewport({ top: 0, right: 89, bottom: 38, left: 52 }, 1200, 704)).toBe(true);
  });
  ...
  it('matches Hyperliquid raw resolution attributes and visible labels', () => {
